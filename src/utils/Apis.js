...
export const BASE_URL = 'https://roadrescue.vercel.app';


export async function getHeaders() {
  let token = await getItem('token');
  if (!token) {
    token = (await getItem('driverToken')) || (await getItem('vendorToken'));
  }
  console.log("token in api", token);
  if (token) {
    return {
      Authorization: 'Bearer ' + token,
      // Authorization: 'Bearer ' + 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6IjY5YjdmMzAyYTY3N2M3MjUyNjgxZWU0MiIsImlhdCI6MTc3NTQ1OTcxN30.LvDN5nKGsXXXQdDCY9J3QVUO60MX6IBmFn1ShgzOAIM',
...
export async function apiReq(
  ...
  return new Promise(async (res, rej) => {
    ...
    if (normalizedMethod === 'get' || normalizedMethod === 'delete') {
      ...
      apiCall = axios[normalizedMethod](url, { headers, params });
    } else {
      apiCall = axios[normalizedMethod](url, data, { headers });
    }
